...
class CourseModel {
  ...
  String getTitle() {
    ...
    //           .Get
    //           .locale
    //           ?.languageCode /* locator<AppLanguage>().currentLanguage*/ ) {
    //     title = element.title ?? '';
    //   }
    // }
  ...
  CourseModel copyWith({
    ...
    return CourseModel(
      ...
      // salesCountNumber: salesCountNumber ?? this.salesCountNumber,
      // liveWebinarStatus: liveWebinarStatus ?? this.liveWebinarStatus,
      // authHasBought: authHasBought ?? this.authHasBought,
      // sales: sales ?? this.sales,
      isFavorite: isFavorite ?? this.isFavorite,
      // expired: expired ?? this.expired,
      // expireOn: expireOn ?? this.expireOn,
      priceString: priceString ?? this.priceString,
      // bestTicketString: bestTicketString ?? this.bestTicketString,
      price: price ?? this.price,
      // tax: tax ?? this.tax,
      // taxWithDiscount: taxWithDiscount ?? this.taxWithDiscount,
      // bestTicketPrice: bestTicketPrice ?? this.bestTicketPrice,
      discountPercent: discountPercent ?? this.discountPercent,
      // coursePageTax: coursePageTax ?? this.coursePageTax,
      priceWithDiscount: priceWithDiscount ?? this.priceWithDiscount,
      discountAmount: discountAmount ?? this.discountAmount,
      // activeSpecialOffer: activeSpecialOffer ?? this.activeSpecialOffer,
      ...
      classSemester: classSemester ?? this.classSemester,
      // capacity: capacity ?? this.capacity,
      // reservedMeeting: reservedMeeting ?? this.reservedMeeting,
      // reservedMeetingUserTimeZone:
      //     reservedMeetingUserTimeZone ?? this.reservedMeetingUserTimeZone,
...
class RateType {
  ...
  RateType.fromJson(Map<String, dynamic> json) {
    contentQuality = double.tryParse(
      json['content_quality']?.toString() ?? '0',
    );
    instructorSkills = double.tryParse(
      json['instructor_skills']?.toString() ?? '0',
    );
    purchaseWorth = double.tryParse(json['purchase_worth']?.toString() ?? '0');
    supportQuality = double.tryParse(
      json['support_quality']?.toString() ?? '0',
    );
  }
